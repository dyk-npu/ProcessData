...
def face_edge_adj(shape):
    ...
    edgeFace_IncM = {}
    edge_dict = {}
    seen_edges = set()  # 在这里初始化seen_edges集合
    ...
    for edge in shape.edges():
        ...
        try:
            ...
            edge_hash = id(tshape)  # 更稳定的几何标识符
            if edge_hash in seen_edges:
                print("跳过重复边")
                continue
            seen_edges.add(edge_hash)
        except:
            # 如果获取底层几何失败，使用简单哈希作为备选
            edge_hash = hash(edge)
            if edge_hash in seen_edges:
                continue
            seen_edges.add(edge_hash)
        ...
        try:
            ...
            if "Edge doesn't belong to face" in str(e):
                print("跳过孤立边")
                continue  # 跳过孤立边
            else:
                raise
...
def array_from_Array1OfReal(array):
    """处理OCC Array1OfReal数据结构"""
    if array is None:
        return np.empty(0)

    values = []
    for i in range(array.Lower(), array.Upper() + 1):
        values.append(array.Value(i))
    return np.asarray(values)


def get_nurbs_data(face, max_ctrl_setting, max_uvlength_setting):
    try:
        ...
        u_knotvector_raw = array_from_Array1OfReal(face_nurbs.UKnotSequence())  # 这里修改为Array1OfReal
        v_knotvector_raw = array_from_Array1OfReal(face_nurbs.VKnotSequence())  # 这里修改为Array1OfReal

        # 检查是否超过节点向量长度限制
        if max(len(u_knotvector_raw), len(v_knotvector_raw)) > max_uvlength_setting:
            return None  # 这里改为返回None

        # 归一化节点向量时避免除以0的错误
        u_min = u_knotvector_raw.min()
        u_max = u_knotvector_raw.max()
        u_range = u_max - u_min
        if u_range > 1e-6:  # 添加安全检查防止除以0
            u_knotvector = (u_knotvector_raw - u_min) / u_range
        else:
            u_knotvector = np.zeros_like(u_knotvector_raw)  # 特殊情况处理

        v_min = v_knotvector_raw.min()
        v_max = v_knotvector_raw.max()
        v_range = v_max - v_min
        if v_range > 1e-6:  # 添加安全检查防止除以0
            v_knotvector = (v_knotvector_raw - v_min) / v_range
        else:
            v_knotvector = np.zeros_like(v_knotvector_raw)  # 特殊情况处理
        ...
        print(f"Error in get_nurbs_data: {e}")
        return None  # 返回None而不是整数