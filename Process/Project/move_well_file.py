...
# 1. CSV文件的完整路径
csv_file_path = "/data_hdd/dev01/dyk/dyk_data/log/label_file_acc.csv"

# 2. 存放 .step 文件的源文件夹路径
step_source_dir = "/data_hdd/dataset/GFR_Dataset_Final"

# 3. 存放 .pkl 标签文件的源文件夹路径
label_source_dir = "/data_hdd/dataset/GFR_data_slim_Final"

# 4. 您希望将文件复制到的主目标文件夹路径
#    脚本会自动在此文件夹下创建 'step' 和 'label' 子文件夹
main_destination_dir = "/data_hdd/dev01/dyk/dyk_data/GFR_dataset_0.95_yb"
...
def filter_and_copy_files_separated(csv_path, step_src, label_src, main_dest):
    ...
    for file_name, group in grouped:
        # 检查该组内'准确率'列的所有值是否都大于0.95并且小于等于1
        # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
        if ((group['准确率'] > 0.95) & (group['准确率'] <= 1)).all():
        # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
            print(f"\n[符合条件]: {file_name} 的所有子特征准确率均在 (0.95, 1] 区间内，准备复制文件...")
