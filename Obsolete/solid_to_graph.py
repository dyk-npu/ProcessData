...
def process_one_file(arguments):
    ...
    bin_file_path = output_path.joinpath(f"{fn_stem}.bin_global")

    # 如果.bin文件已经存在，则跳过此文件
    if bin_file_path.exists():
        print(f"File {fn_stem}.bin_global already exists, skipping...")
        return
    ...
    for solid in solids:
        try:
            ...
            dgl_graph_file = str(output_path.joinpath(f"{fn_stem}.bin_global"))
            dgl.data.utils.save_graphs(dgl_graph_file, [graph])
        except IndexError as e:
            print(f"Error processing file {fn}: {e}. Skipping this solid.")
            continue
        except Exception as e:
            print(f"Unexpected error occurred: {e}. Skipping this solid.")
            continue
    ...
    #     solid, args.curv_u_samples, args.surf_u_samples, args.surf_v_samples
    # )
    # dgl.data.utils.save_graphs(str(output_path.joinpath(fn_stem + ".bin_global")), [graph])
