...
def rename_and_move_files(folder_path, new_folder_name="step2", prefix="double_"):
    """
    ...
    step_files = [f for f in all_files if f.startswith(prefix) and f.endswith(".step")]

    # 提取文件名中的数字，并按数字排序
    file_data = []
    for filename in step_files:
        match = re.search(rf"{prefix}(\d+)\.step", filename)  # 匹配数字部分
        if match:
    ...
    # 按新顺序重命名文件并移动到新文件夹
    for new_index, (old_number, old_filename) in enumerate(tqdm(file_data, desc="Processing Files"), start=8000):
        # 构造新的文件名
        new_filename = f"{prefix}{new_index}.step"
