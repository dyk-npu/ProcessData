...
"""
B-rep NURBS曲面控制点与重采样点可视化脚本 - v4

功能:
    加载一个STEP文件，并对指定的面进行可视化。
    - 原始控制点显示为红色。
    - 原始控制多边形显示为黑色。
    - 经过均匀重采样（插值）得到的新控制点，将作为绿色大球叠加显示，
      用于直观地验证采样算法的正确性。
"""
...
# MODIFIED: 引入scipy用于插值。如果未安装，请运行: pip install scipy
try:
    from scipy.ndimage import map_coordinates
except ImportError:
    print("错误: 本脚本需要 'scipy' 库来进行均匀重采样。")
    print("请使用 'pip install scipy' 命令进行安装。")
    sys.exit(1)
...
def visualize_control_points(viewer, shape,
                             ...
                             show_hull=True,
                             show_resampled_points=True, # MODIFIED: 新增参数
                             original_point_color=(1, 0, 0),    # 红色: 原始控制点
                             resampled_point_color=(0, 1, 0),   # 绿色: 重采样后的点
                             point_scale=2,
                             resampled_point_scale=2, # 让重采样点更大更明显
                             hull_color=(0, 0, 0),              # 黑色
                             highlight_color=(0.1, 0.6, 1.0),
                             default_color=(0.8, 0.8, 0.8),
                             transparency=0.7):
    """
    在给定的viewer中绘制几何实体指定面的控制点、控制多边形以及重采样点。
    """
    ...
    for i, face in faces_to_process:
        ...
        # --- MODIFIED: 在此处执行与数据处理脚本相同的均匀重采样逻辑 ---
        if show_resampled_points:
            print(f"  - 正在为面 {i} 计算重采样点...")
            size_u, size_v = poles_np.shape[0], poles_np.shape[1]
            
            grid_u, grid_v = np.mgrid[0:TARGET_CTRL_PNTS, 0:TARGET_CTRL_PNTS]
            rescaled_u = (grid_u / (TARGET_CTRL_PNTS - 1)) * (size_u - 1) if TARGET_CTRL_PNTS > 1 else 0
            rescaled_v = (grid_v / (TARGET_CTRL_PNTS - 1)) * (size_v - 1) if TARGET_CTRL_PNTS > 1 else 0
            coords = np.stack([rescaled_u, rescaled_v])

            # 对 X, Y, Z 三个坐标通道分别进行插值
            resampled_channels = [map_coordinates(poles_np[:, :, channel], coords, order=1, mode='nearest') for channel in range(3)]
            resampled_poles_np = np.stack(resampled_channels, axis=-1)
            
            # 显示重采样后的点
            resampled_flat = resampled_poles_np.reshape(-1, 3)
            viewer.display_points(resampled_flat, color=resampled_point_color, scale=resampled_point_scale)
            print(f"    ... {len(resampled_flat)} 个重采样点（绿色）已添加。")
        # --- 修改结束 ---
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("B-rep NURBS曲面控制点与重采样点可视化脚本")
    parser.add_argument("--input_file", type=str, help="要可视化的STEP文件路径")
    args = parser.parse_args()

    # 如果不通过命令行提供文件，则使用此处的默认文件路径
    if not args.input_file:
        # !!! 在这里修改你的默认STEP文件路径 !!!
        args.input_file = r"E:\CAD数据集\项目\GFR_Dataset_Final\GFR_02848.step" 
